...
function isUsableForContent(material: MaterialFile): boolean {
...
function isUsableForStructure(material: MaterialFile): boolean {
  ...
  return reliability.usableForStructure
}
...
export function buildBlueprint(
  ...
  const curriculumMaterials = selectStrongestMaterials(
    materials.filter((material) => isUsableForContent(material))
  )

  const exemplarMaterials = selectStrongestMaterials(
    materials.filter((material) => isUsableForStructure(material))
  )
