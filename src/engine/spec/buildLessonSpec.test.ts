...
import { buildLessonSpec } from "./buildLessonSpec"
...
const blueprint = {
  ...
  structure: {
    ...
    templateShell: {
      ...
      timingShell: ["Opening", "Mini-lesson", "Guided Practice"],
      teacherMoveShell: ["teacher model", "guided support"],
...
describe("buildLessonSpec", () => {
  it("builds lesson spec content from normalized blueprint values", () => {
    const spec = buildLessonSpec(blueprint)
    const joined = JSON.stringify(spec)
