...
export function buildLessonSpec(input: LessonInput, blueprint?: LessonBlueprint | null): LessonSpec {
  ...
  const frameworkApplied = context.teacherLed && originalFramework === "clickableHub" ? "linear" : originalFramework;
  ...
  const cues = (blueprint.exemplar?.presenterCues || []).slice(0, context.teacherLed ? 4 : 10);
  ...
  if (frameworkApplied === "clickableHub" && !context.teacherLed) {
  ...
  if (frameworkApplied === "guidepost" && !context.teacherLed) {
