...
import { extractStandardCode, isKnownStandardDescription, normalizeStandardValue } from "../shared/standards"
...
function selectVocabulary(lines: string[]): string[] {
  ...
  return takeBestMatches(
    ...
    (line) =>
      ...
      !isStandardLikeTeacherContentLine(line) &&
      !looksLikeSongOrRoutineLine(line) &&
      (
        containsAny(line, [
...
function isStandardLikeTeacherContentLine(line: string): boolean {
  const normalized = normalizeStandardValue(line)
  if (!normalized) {
    return false
  }

  if (extractStandardCode(normalized)) {
    return true
  }

  const lower = normalized.toLowerCase()
  return (
    isKnownStandardDescription(normalized) ||
    (/^(standard|standards|benchmark|benchmarks)\b/.test(lower)) ||
    (/(demonstrate|identify|retell|ask and answer|sort common words|read high-frequency words|use new vocabulary)/.test(lower) &&
      /(phonological awareness|main topic|key details|author's purpose|vocabulary|high-frequency words|high frequency words)/.test(lower))
  )
}

function looksLikeSongOrRoutineLine(line: string): boolean {
  const lower = line.toLowerCase()
  return /\bsong\b/.test(lower) || /\breview\b/.test(lower) || /\bteacher scripting\b/.test(lower)
}
