...
describe("material analysis signals", () => {
  it("detects richer curriculum signals from structured lesson text", async () => {
    ...
    expect(result.analysis.tags).toEqual(expect.arrayContaining(["standards", "vocabulary", "word work", "practice", "instruction", "phonics", "long a"]))
  })

  it("detects richer exemplar signals from lesson-delivery text", async () => {
    const result = await analyzeMaterial({
      materialId: "ex-1",
      role: "exemplar",
      name: "exemplar.txt",
    ...
    expect(result.analysis.tags).toEqual(expect.arrayContaining(["slide flow", "pacing", "teacher prompts", "layout", "structure"]))
  })
})
